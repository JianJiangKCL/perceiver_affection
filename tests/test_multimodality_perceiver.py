...
from models.modalities import modality_encoding
...
def test_multimodality_forward_image_video(image_inputs, video_inputs, audio_inputs, targets):

    # input_dim = 3 * ((6 * 2) + 1) + 3 = 39
    ...
    model = MultiModalityPerceiver(
        ...
        cross_dim_head=64,
        latent_dim_head=64,
        num_outputs=num_outputs,  # output number of classes
        attn_dropout=0.,
        ff_dropout=0.,
...
image_inputs = torch.rand(size=(1, 128, 128, 3), requires_grad=True)
video_inputs = torch.rand(size=(1, 32, 128, 128, 3), requires_grad=True)
audio_inputs = torch.rand(size=(1, 44100, 1), requires_grad=True)
test_multimodality_forward_image_video(image_inputs, video_inputs, audio_inputs, None)
print('done')
